...
public class ProtocolPayController {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/saveProtocolPay")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("协议支付,将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("协议支付,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("协议支付,将参数封装成tranData,处理中");
            ...
            logger.info("协议支付,返回,{}", value);
            result = json.toJson(value);
            logger.info("协议支付,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("协议支付,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}