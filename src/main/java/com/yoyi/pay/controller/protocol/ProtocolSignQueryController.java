...
public class ProtocolSignQueryController {
     ...
     * @return
     */
    @RequestMapping(value = "/protocolSignQuery")
    public String toJsp() {
        return "protocol/protocol_sign_query";
    }

    /**
     * 签约结果查询,将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/saveProtocolSignQuery")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("签约结果查询,将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("签约结果查询,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("签约结果查询,将参数封装成tranData,处理中");
            ...
            logger.info("签约结果查询,返回,{}", value);
            result = json.toJson(value);
            logger.info("签约结果查询,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("签约结果查询,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}