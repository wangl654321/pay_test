...
public class WithHoldRefundOrderController {
    ...
    public String toJsp() {
        return "withholding/refund_order";
    }
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/saveRefundOrder")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("代扣退款,将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("代扣退款,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("代扣退款,将参数封装成tranData,处理中");
            ...
            logger.info("代扣退款,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("代扣退款,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}