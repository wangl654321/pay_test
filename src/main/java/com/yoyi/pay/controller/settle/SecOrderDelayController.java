...
public class SecOrderDelayController {
     ...
     * @return
     */
    @RequestMapping(value = "/secOrderDelay")
    public String toJsp() {
        return "settle/sec_order_delay";
    }

    /**
     * (结算通)延期,将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/toSecOrderDelay")
    public String registered(HttpServletRequest request) {
        ...
        for (String keys : parameterMap.keySet()) {
            ...
            logger.info("(结算通)延期,将参数封装成tranData,接口名称,{}", interfaceName);
        ...
        try {
            ...
            logger.info("(结算通)延期,将参数封装成tranData,xml,{}", xml);
            ...
            logger.info("(结算通)延期,返回结果,{}", value);
        } catch (Exception e) {
            logger.error("(结算通)延期,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
    ...
    private Map<String, String> getStringStringMap(String merchantId, String interfaceName, String version,
        ...
        logger.info("(结算通)延期,封装参数,{}", merchantId);
