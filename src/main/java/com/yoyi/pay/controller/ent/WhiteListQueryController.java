...
@Controller
@RequestMapping("/ent")
public class WhiteListQueryController {
     ...
     * @return
     */
    @RequestMapping(value = "/whiteListQuery")
    public String toJsp() {
        return "ent/white_list_query";
    }

    /**
     * 白名单会员查询,将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/saveWhiteListQuery")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("白名单会员查询,将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("白名单会员查询,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("白名单会员查询,将参数封装成tranData,处理中");
            ...
            logger.info("白名单会员查询,调取接口返回密文,{}", value);
            String decode = Base64Utils.decode(value);
            logger.info("白名单会员查询,返回,{}", decode);

            Map<String, String> map = YoYiPayUtil.xmlParse(decode);
            result = json.toJson(map);
            logger.info("白名单会员查询,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("白名单会员查询,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}
