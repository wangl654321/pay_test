...
@Controller
@RequestMapping("/ent")
public class BindWhiteCardController {
     ...
     * @return
     */
    @RequestMapping(value = "/bindWhiteCard")
    public String toJsp() {
        return "ent/bind_white_card";
    }

    /**
     * 会员白名单卡绑定,将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/saveBindWhiteCard")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("会员白名单卡绑定,将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("会员白名单卡绑定,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("会员白名单卡绑定,将参数封装成tranData,处理中");
            ...
            logger.info("会员白名单卡绑定,调取接口返回密文,{}", value);
            String decode = Base64Utils.decode(value);
            logger.info("会员白名单卡绑定,返回,{}", decode);

            Map<String, String> map = YoYiPayUtil.xmlParse(decode);
            result = json.toJson(map);
            logger.info("会员白名单卡绑定,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("会员白名单卡绑定,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}
