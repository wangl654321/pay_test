...
@Controller
@RequestMapping("/ent")
public class UnCorAcctBankAuthController {
     ...
     * @return
     */
    @RequestMapping(value = "/unCorAcctBankAuth")
    public String toJsp() {
        return "ent/un_cor_acct_bank_auth";
    }

    /**
     * 企业账户打款认证(未绑定银行账户),将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/saveUnCorAcctBankAuth")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("企业账户打款认证(未绑定银行账户),将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("企业账户打款认证(未绑定银行账户),将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("企业账户打款认证(未绑定银行账户),将参数封装成tranData,处理中");
            ...
            logger.info("企业账户打款认证(未绑定银行账户),调取接口返回密文,{}", value);
            String decode = Base64Utils.decode(value);
            logger.info("企业账户打款认证(未绑定银行账户),返回,{}", decode);

            Map<String, String> map = YoYiPayUtil.xmlParse(decode);
            result = json.toJson(map);
            logger.info("企业账户打款认证(未绑定银行账户),调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("企业账户打款认证(未绑定银行账户),将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}
