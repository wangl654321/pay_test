...
@Controller
@RequestMapping("/ent")
public class UnBindWhiteCardController {
     ...
     * @return
     */
    @RequestMapping(value = "/unBindWhiteCard")
    public String toJsp() {
        return "ent/un_bind_white_card";
    }

    /**
     * 会员白名单卡状态修改,将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/saveUnBindWhiteCard")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("会员白名单卡状态修改,将参数封装成tranData,请求地址,{}", regUrl);
        ...
        try {
            ...
            logger.info("会员白名单卡状态修改,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("会员白名单卡状态修改,将参数封装成tranData,处理中");
            ...
            logger.info("会员白名单卡状态修改,调取接口返回密文,{}", value);
            String decode = Base64Utils.decode(value);
            logger.info("会员白名单卡状态修改,返回,{}", decode);

            Map<String, String> map = YoYiPayUtil.xmlParse(decode);
            result = json.toJson(map);
            logger.info("会员白名单卡状态修改,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("会员白名单卡状态修改,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}
