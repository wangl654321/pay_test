package com.yoyi.pay.controller.single;
...
public class SinglePayController {
    ...
    public String toJsp() {
        return "single/singe_pay";
    }

    /**
     * 单笔汇款接口将参数封装成tranData
     *
     * @param request
     * @return
     */
    @ResponseBody
    @RequestMapping(value = "/saveSinglePay")
    public String registered(HttpServletRequest request) {
        ...
        logger.info("单笔汇款接口,将参数封装成tranData,请求地址,{}", regUrl);

        //接收页面参数封装xml在base64加密
        for (String keys : parameterMap.keySet()) {
            if ("regUrl".equals(keys) || "merchantId".equals(keys) || "interfaceName".equals(keys) || "version".equals(keys)) {
                continue;
            }
        ...
        try {
            ...
            logger.info("单笔汇款接口,将参数封装成tranData,xml,{}", xml);
            //订单签名数据
            ...
            logger.info("单笔汇款接口,将参数封装成tranData,处理中");
            ...
            String post = instance.post(regUrl, sendParams, null);
            Map<String,String> map = json.fromJson(post, Map.class);
            result = json.toJson(map);
            logger.info("单笔汇款接口,调取接口返回明文,{}", result);
        } catch (Exception e) {
            logger.error("单笔汇款接口,将参数封装成tranData,异常{}", e);
        }
        return result;
    }
}